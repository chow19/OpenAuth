<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="IPQC_Route.aspx.cs" Inherits="QMS_WebSite.IPQC_Route" %>
...
<head>
    ...
    <script>
        ...
        function getData(curPage, keyWork) {
            $.ajax({
                url: "Handler/IPQCRountCheck.ashx?FunType=getIPQCRouteCheckTodo&curPage=" + curPage + "&keyWork=" + keyWork,
                type: "Get",
                cache: false,
        ...
        function getDoneData(curPage, keyWork) {
            $.ajax({
                url: "Handler/IPQCRountCheck.ashx?FunType=getIPQCRouteCheckDone&curPage=" + curPage + "&keyWork=" + keyWork,
                type: "Get",
                cache: false,
                success: function (data) {
        ...
        function AddCheckDetail(RouteId, SpecificationName) {
            ...
            openNewWindow(RouteId, SpecificationName);
        }

        function PrintLabel(WFSteptId, IsDone) {
            $.modal({
                title: "确认",
                text: "确认要打印IPQC巡检打印标签?",
                buttons: [
                  {
                      text: "确定", onClick: function () {  //点击确认
                          $.ajax({
                              url: "Handler/IPQCRountCheck.ashx?FunType=printLabel&WFSteptId=" + WFSteptId,
                              type: "Get",
                              cache: false,
                              success: function (data) {
                                  if (data.result = "0") { 
        ...
        function runPrint(YBBQ, WFSteptId) {
            ...
            $.ajax({
                url: "Handler/IPQCRountCheck.ashx?FunType=getPrintContentCode&WFSteptId=" + WFSteptId + "&YBBQ=" + YBBQ,
                type: "Get",
                cache: false,
        ...
        function showCheckDetail(RouteId, SpecificationName) {
            if (SpecificationName.length <= 0) {
                $M.alert("数据错误");
                return;
            }
            if (SpecificationName.indexOf('模切') > -1) {
                $M.open("IPQCRoute/show/Diecut_Show.aspx?RouteId=" + RouteId, "模切");
            } else if (SpecificationName.indexOf('贴合') > -1) {
                $M.open("IPQCRoute/show/Bonded_Show.aspx?RouteId=" + RouteId, "切合");
            } else if (SpecificationName.indexOf('检片') > -1) {
                $M.open("IPQCRoute/show/Examination_Show.aspx?RouteId=" + RouteId, "检片");
            }
        }

        function openNewWindow(RouteId, SpecificationName) {
       
              if (SpecificationName.indexOf('模切') > -1) {
                  $M.open("IPQCRoute/add/Diecut_Check.aspx?RouteId=" + RouteId, "模切");
            } else if (SpecificationName.indexOf('贴合') > -1) {
                $M.open("IPQCRoute/add/Bonded_Check.aspx?RouteId=" + RouteId, "切合");
            } else if (SpecificationName.indexOf('检片') > -1) {
                $M.open("IPQCRoute/add/Examination_Check.aspx?RouteId=" + RouteId, "检片");
            } else {
                $M.alert("未知工序");
            }
        }
                         ...
                         <div class="weui-cell__bd">
                             ...
                             <div>
                                 ...
                                 {{else if value.IsDone=="0"  }}
                                  <a href="javascript:;" class="weui-btn weui-btn_mini weui-btn_warn" onclick="AddCheckDetail('{{value.IPQCRouteCheckId}}','{{value.SpecificationName}}');" id="JY{{value.IPQCRouteCheckId}}">检验</a>
                                 {{/if}} 
                             </div>
                         </div>
                     ...
                     <div class="weui-cell">
                         <div class="weui-cell__bd">
                             ...
                             <div>
                                 ...
                                 {{/if}}
                                 <a href="javascript:;" class="weui-btn weui-btn_mini weui-btn_primary" onclick="showCheckDetail('{{value.IPQCRouteCheckId}}','{{value.SpecificationName}}');">查看详细</a>
...
<body>
    ...
    <div class="weui-tab">
        <div class="weui-navbar">
            <a class="weui-navbar__item weui-bar__item--on" href="#tab1">IPQC巡检
            </a>
